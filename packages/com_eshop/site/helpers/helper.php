...
class EshopHelper
{
	...
	public static function getDiscountPrices($productId)
	{
		...
		{
			if ($specialPrice = self::getSpecialPrice($productId, $discountPrices[$i]->price))
			{
	...
	public static function getAdminQuoteEmailBody($row)
	{
		...
		$view->quoteProducts         = $quoteProducts;
		$view->row                   = $row;
		ob_start();
		$view->display();
	...
	public static function getAdminEmailBody($row)
	{
		...
		$view                        = new JViewLegacy($viewConfig);
		$orderProducts               = self::getOrderProducts($row->id);
		$view->orderProducts         = $orderProducts;
		$orderTotals                 = self::getOrderTotals($row->id);
		$view->orderTotals           = $orderTotals;
		$view->row                   = $row;
		ob_start();
		$view->display();
		$text = ob_get_contents();
	...
	public static function getCustomerEmailBody($row)
	{
		...
		$view->showDownloadLink = $showDownloadLink;
	...
	public static function getNotificationEmailBody($row, $orderStatusFrom, $orderStatusTo)
	{
		...
		$orderProducts               = self::getOrderProducts($row->id);
		$view->orderProducts         = $orderProducts;
		$view->orderTotals           = self::getOrderTotals($row->id);
		$view->row                   = $row;
		$hasDownload                 = false;
	...
	{
		echo '<div class="copyright" style="text-align:center;margin-top: 5px;"><a href="https://www.joomdonation.com" target="_blank"><strong>EShop</strong></a> version 2.6.2, Copyright (C) 2012-2013 <a href="http://joomdonation.com" target="_blank"><strong>Ossolution Team</strong></a></div>';
	}
	...
	public static function getInvoiceProducts($orderId)
	{
		...
		$view->order_id       = $orderId;
		$view->order_products = $orderProducts;
		$view->order_total    = $orderTotals;
		ob_start();
		$view->display();
		$text = ob_get_contents();
		ob_end_clean();
	...
	public static function generateProductPDF($productId)
	{
		...
		$filename = JFile::makeSafe($product->product_name) . '_' . $product->product_sku . '.pdf';
	...
	{
		$db                          = JFactory::getDbo();
		$viewConfig['name']          = 'productpdf';
		$viewConfig['base_path']     = JPATH_ROOT . '/components/com_eshop/pdftemplates';
		$viewConfig['template_path'] = JPATH_ROOT . '/components/com_eshop/pdftemplates';
		$viewConfig['layout']        = 'default';
		$view                        = new JViewLegacy($viewConfig);
		$product                     = self::getProduct($productId);
		...
		$product->thumb_image = JUri::base(true) . '/media/com_eshop/products/resized/' . $thumbImage;
		...
		$view->currency          = new EshopCurrency();
		$view->tax               = new EshopTax(self::getConfig());
		$view->product           = $product;
